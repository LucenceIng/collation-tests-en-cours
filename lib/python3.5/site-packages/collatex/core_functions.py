...
def export_alignment_table_as_xml(table):
    ...
    for column in table.columns:
        ...
        #modification du 10/02/2018 : ajout d'un attribut sur les app qui ne contiennent pas de variance
        if not column.variant:
            app.attrib['type']='novariance'
        #fin de la modication du 10/02/2018
        
        for key , value in sorted(column.tokens_per_witness.items()):
            child = etree.Element('rdg')
            child.attrib['wit'] = "#" + key
            
            #modification du 11/02/2018            
            #modification du code : on enlève cette ligne
            #child.text = "".join(str(item.token_data["t"]) for item in value)
            
            #création des 4 lignes suivantes : pour item in value, création d'un sous-élément w, qui a pour attribut l'id du mot et comme valeur le mot, afin de pouvoir réutiliser toutes ces données par la suite
            for item in value:
                subchild = etree.SubElement(child, 'w')            
                subchild.text= item.token_data["t"]
                subchild.attrib['xml:id'] = item.token_data["i"]
                #ajout du 12/02/2018 : pour s'il y a des lemmes : on exécute la collation sur les lemmes, et on garde leur valeur dans un attribut
                subchild.attrib['lemma'] = item.token_data["n"]
            #fin modification 12/02/2018
          #fin de la modification 11/02/2018
          
            app.append(child)
        
        #modification 2 : pour faire apparaître les leçons absentes d'un texte ; test sur le nombre d'éléments présents dans app ; création d'un attribut sur l'app et d'un élément enfant, portant le sigle associé et étant vide -- modification du 12/02/2018
        if len(list(app)) == 1:
            app.attrib['type'] = 'only' + key
            newel = etree.SubElement(app, 'rdg')
            if key == 'A':
                newel.attrib['wit'] = '#B'
            else:
                newel.attrib['wit'] = '#A'
