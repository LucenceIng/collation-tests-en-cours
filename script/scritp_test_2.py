...
#modification pour tests sur p 38
os.listdir('./docs/coll_038')

witnessSet = WitnessSet([(inputFile[0],open('./docs/coll_038/' + inputFile,'rb').read()) for inputFile in os.listdir('./docs/coll_038')])
...
graph = collate(json_input, output='xml', astar=True)
with open("./sortie/038_test.xml", "w") as text_file:
